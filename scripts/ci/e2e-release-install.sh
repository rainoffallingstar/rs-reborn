...
echo "==> installing rvx from the locally built release artifact"
RS_INSTALL_TAG="$TAG" \
RS_INSTALL_BASE_URL="file://$DIST_DIR" \
RS_INSTALL_DIR="$INSTALL_DIR" \
bash "$ROOT_DIR/install.sh" | tee "$TMP_DIR/install.txt"

test -x "$INSTALL_DIR/rvx"
grep -q "installed rvx $TAG" "$TMP_DIR/install.txt"
grep -q "verified sha256" "$TMP_DIR/install.txt"

echo "==> running the installed binary"
"$INSTALL_DIR/rvx" --help | tee "$TMP_DIR/help.txt"
grep -q "Usage:" "$TMP_DIR/help.txt"
"$INSTALL_DIR/rvx" version | tee "$TMP_DIR/version.txt"
grep -q "rvx $TAG" "$TMP_DIR/version.txt"
