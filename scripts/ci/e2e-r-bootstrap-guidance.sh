...
case "$(uname -s)" in
  Darwin)
    grep -q 'next step: install a managed R version with rvx or set rs.toml rscript manually: rvx r install 4.4' "$TMP_DIR/run.txt"
    ;;
  Linux)
    grep -q 'next step: install a managed R version with rvx or set rs.toml rscript manually: rvx r install 4.4' "$TMP_DIR/run.txt"
    ;;
...
grep -q '\[next\] explicitly allow rvx to install R automatically and rerun: RS_AUTO_INSTALL_R=1 rvx run ' "$TMP_DIR/doctor.txt"
case "$(uname -s)" in
  Darwin)
    grep -q '\[next\] install a managed R version with rvx or set rs.toml rscript manually: rvx r install 4.4' "$TMP_DIR/doctor.txt"
    ;;
  Linux)
    grep -q '\[next\] install a managed R version with rvx or set rs.toml rscript manually: rvx r install 4.4' "$TMP_DIR/doctor.txt"
    ;;
